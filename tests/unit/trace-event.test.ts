...
describe("traceEvent", () => {
  ...
  it("should POST event to runs-service", async () => {
    ...
    await traceEvent(
      "run-123",
      { service: "cloudflare-service", event: "test-event", detail: "details here" },
    ...
    expect(body.service).toBe("cloudflare-service");
  ...
  it("should skip when RUNS_SERVICE_URL is not set", async () => {
    ...
    await traceEvent(
      "run-123",
      { service: "cloudflare-service", event: "test" },
      {}
    );
  ...
  it("should forward identity headers when present", async () => {
    process.env.RUNS_SERVICE_URL = "https://runs.test";
    process.env.RUNS_SERVICE_API_KEY = "test-key";
